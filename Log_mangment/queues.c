...
void Dequeue(queue*Q){
    if (isEmptyQueue(Q)||Q==NULL)
    {
        return ;
    }
    else
    {
        DelLogAtTheBeginning(&(Q->Head));
    }   
}
LogEntry* FDequeue(queue*Q){
    if (isEmptyQueue(Q)||Q==NULL)
    {
        return NULL;
    }
    
    LogEntry *Resault=Q->Head;
    Q->Head=Q->Head->next;
    Resault->next=NULL;
    return Resault;
}
void FEnqueue(queue*Q,LogEntry*L){
    if (Q==NULL)
    {
        return;
    }
...
void Peek(queue*Q){
    ...
    {
        displayLogHeader();
        displayLogEntryColumn(Q->Head);
    }
}
bool isFull(queue**S,int size){
    queue*Q=*S;
    queue*Tem=createQueue();
    if (isEmptyQueue(Q))
    {
        return false;
    }
    int counter =0;
    LogEntry*Temp=Q->Head;
    LogEntry*Current;
    while (!=isEmptyQueue(Q))
    {
        FEnqueue(Tem,FDequeue(Q));
        counter++;
    }
    *Q=Tem;
    if (counter==size)
